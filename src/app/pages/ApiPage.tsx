...
const ENDPOINTS: EndpointDef[] = [
  ...
  },
  {
    ...
    examplePath: '/providers',
    exampleQuery: 'finance_coverage=pending&fiscal_year=2024-25&limit=10',
  },
  {
    group: 'Providers',
    method: 'GET', path: '/provider-finance-coverage',
    summary: 'Provider finance coverage',
    description: 'Provider-year coverage matrix for all 304 HESA provider rows across 2015-16 to 2024-25. Pending rows are null and excluded from aggregates.',
    params: [
      { name: 'fiscal_year', in: 'query', type: 'string', required: false, description: 'Fiscal year (default latest)', example: '2024-25' },
      { name: 'source_status', in: 'query', type: 'string', required: false, description: 'verified or pending', example: 'pending' },
      { name: 'provider_id', in: 'query', type: 'string', required: false, description: 'Provider universe id', example: 'nottingham' },
      { name: 'limit', in: 'query', type: 'integer', required: false, description: 'Results per page (1-304)', example: '20' },
    ],
    examplePath: '/provider-finance-coverage',
    exampleQuery: 'fiscal_year=2024-25&source_status=pending',
  },
  {
    group: 'Providers',
    method: 'GET', path: '/provider-source-coverage',
    summary: 'Provider source coverage',
    description: 'Provider-domain coverage rows for students, outcomes, staff and estates. This endpoint makes source gaps explicit.',
    params: [
      { name: 'domain', in: 'query', type: 'string', required: false, description: 'students, outcomes, staff or estates', example: 'staff' },
      { name: 'source_status', in: 'query', type: 'string', required: false, description: 'verified or pending', example: 'pending' },
      { name: 'provider_id', in: 'query', type: 'string', required: false, description: 'Provider universe id', example: 'oxford' },
      { name: 'limit', in: 'query', type: 'integer', required: false, description: 'Results per page', example: '20' },
    ],
    examplePath: '/provider-source-coverage',
    exampleQuery: 'domain=staff&limit=10',
  },
  {
    group: 'Student Finance',
    method: 'GET', path: '/national-student-finance',
    summary: 'National student finance',
    description: 'SLC England support statistics and DfE student-loan forecasts. Forecast rows are labelled and excluded from official observed aggregates.',
    params: [
      { name: 'geography', in: 'query', type: 'string', required: false, description: 'Filter by geography', example: 'England' },
      { name: 'category', in: 'query', type: 'string', required: false, description: 'Filter by support category', example: 'maintenance-loans' },
      { name: 'source_status', in: 'query', type: 'string', required: false, description: 'verified, provisional or forecast', example: 'forecast' },
      { name: 'academic_year', in: 'query', type: 'string', required: false, description: 'Filter by academic year', example: '2024-25' },
      { name: 'financial_year', in: 'query', type: 'string', required: false, description: 'Filter by financial year', example: '2024-25' },
    ],
    examplePath: '/national-student-finance',
    exampleQuery: 'category=maintenance-loans',
  },
  {
    group: 'Sector',
  ...
  {
    group: 'Sector',
    method: 'GET', path: '/system-risk',
    summary: 'UK HE system risk',
    description: 'Transparent early-warning snapshot across institution finances, graduate employment, labour demand and recruitment exposure, including methods and sources.',
    params: [],
    examplePath: '/system-risk',
  },
  {
    group: 'Estates',
    method: 'GET', path: '/estate-metrics',
    summary: 'Normalised estates metrics',
    description: 'Analysis-ready HESA DT042 provider-year-metric rows. Optional, blank and suppressed fields remain absent rather than being inferred as zero.',
    params: [
      { name: 'academic_year', in: 'query', type: 'string', required: false, description: 'Academic year', example: '2024-25' },
      { name: 'institution_id', in: 'query', type: 'string', required: false, description: 'HEStats institution id or UKPRN', example: 'nottingham' },
      { name: 'metric_id', in: 'query', type: 'string', required: false, description: 'Normalised metric key', example: 'energy_consumption_kwh' },
      { name: 'category', in: 'query', type: 'string', required: false, description: 'buildings, energy, water, emissions, waste or condition', example: 'energy' },
      { name: 'limit', in: 'query', type: 'integer', required: false, description: 'Results per page (max 2,000)', example: '50' },
    ],
    examplePath: '/estate-metrics',
    exampleQuery: 'academic_year=2024-25&institution_id=nottingham',
  },
  {
    group: 'Rankings',
    method: 'GET', path: '/rankings',
    summary: 'League table',
    description: 'Ranked list of institutions by a financial metric or the coverage-aware overall evidence score. Supports filtering by nation and sorting direction.',
    params: [
      { name: 'metric', in: 'query', type: 'string', required: true, description: 'Metric key, including overall (see /metrics for the financial list)', example: 'overall' },
    ...
    exampleQuery: 'metric=overall&limit=10',
  },
  {
    group: 'Rankings',
