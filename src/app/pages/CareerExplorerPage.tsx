  ...
  return (
    <div>
      ...
      <div className="mt-3 p-3 border" style={{ backgroundColor: 'var(--bg-2)', borderColor: 'var(--border)', borderRadius: 3, borderLeft: '3px solid var(--muted)' }}>
        <p style={{ color: 'var(--muted)', fontSize: 10.5 }}>AI impact records are external analysis based on McKinsey Global Institute and Oxford Future of Work research. They are not official statistics and represent task exposure, not job losses.</p>
      </div>
    </div>
  )
}
...
function StepSummary({ degree, onReset }: { degree: Degree; onReset: () => void }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: 'var(--muted)', fontSize: 10.5 }}>Figures combine source-backed sector averages and external-analysis guidance records. Individual outcomes vary by institution, degree class, and geography.</p>
    </div>
  )
}
